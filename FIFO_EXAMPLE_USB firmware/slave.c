...
{ // Called once at startup
  ...
  PORTACFG |= 0x80;		//Set PA7 as FLAGD pin
  SYNCDELAY;
  ...
  SYNCDELAY;
	...
	//JTAG Enable and SYNC signal
	OEA|=0x02; //Declare PA.1 as output
	SYNCDELAY;
	IOA|=0x00; //output 0 on PA.1
	EZUSB_Delay(1); // keep low for ~1ms
	SYNCDELAY;
...
{ // Called repeatedly while the device is idle
 		...
 		if(!(IOA << 7 & 0x80)) //Check PA.0 is '1', FPGA is ready
		{
			done_frm_fpga = 1;
		}
		if ((done_frm_fpga) && !(IOA << 7 & 0x80))
		{
			IFCONFIG = 0x03;  //external clock input, Slave FIFO interface
			SYNCDELAY;
		
			IOA|=0x02; //output 1 on PA.1...SYNC signal is HIGH 
			SYNCDELAY;
			done_frm_fpga = 0;			
		}
